...
int FuzzerTestOneInput(const uint8_t *buf, size_t len)
{
    ...
    OPENSSL_assert(VR_BN_div(b3, b4, b1, b2, ctx));
    if (VR_BN_is_zero(b1))
        success = VR_BN_is_zero(b3) && VR_BN_is_zero(b4);
    else if (VR_BN_is_negative(b1))
        success = (VR_BN_is_negative(b3) != VR_BN_is_negative(b2) || VR_BN_is_zero(b3))
            && (VR_BN_is_negative(b4) || VR_BN_is_zero(b4));
    else
        success = (VR_BN_is_negative(b3) == VR_BN_is_negative(b2)  || VR_BN_is_zero(b3))
            && (!VR_BN_is_negative(b4) || VR_BN_is_zero(b4));
    OPENSSL_assert(VR_BN_mul(b5, b3, b2, ctx));
    OPENSSL_assert(VR_BN_add(b5, b5, b4));

    success = success && VR_BN_cmp(b5, b1) == 0;
    if (!success) {
        ...
        putchar('\n');
        printf("%d %d %d %d %d %d %d\n", VR_BN_is_negative(b1),
               VR_BN_is_negative(b2),
               VR_BN_is_negative(b3), VR_BN_is_negative(b4), VR_BN_is_zero(b4),
               VR_BN_is_negative(b3) != VR_BN_is_negative(b2)
               && (VR_BN_is_negative(b4) || VR_BN_is_zero(b4)),
               VR_BN_cmp(b5, b1));
        puts("----\n");
    }
