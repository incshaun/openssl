...
# endif
 ...
 opthelp:
    ...
    VR_BIO_printf(bio_err, "read DSA key\n");
    {
        EVP_PKEY *pkey;
    ...
    if (modulus) {
        ...
        VR_DSA_get0_key(dsa, &pub_key, NULL);
        VR_BIO_printf(out, "Public Key=");
        VR_BN_print(out, pub_key);
        VR_BIO_printf(out, "\n");
    }
    ...
    VR_BIO_printf(bio_err, "writing DSA key\n");
    if (outformat == FORMAT_ASN1) {
        if (pubin || pubout) {
            i = VR_i2d_DSA_PUBKEY_bio(out, dsa);
        } else {
            assert(private);
            i = VR_i2d_DSAPrivateKey_bio(out, dsa);
        }
    } else if (outformat == FORMAT_PEM) {
        if (pubin || pubout) {
            i = VR_PEM_write_bio_DSA_PUBKEY(out, dsa);
        } else {
            assert(private);
            i = VR_PEM_write_bio_DSAPrivateKey(out, dsa, enc,
                                            NULL, 0, NULL, passout);
        }
