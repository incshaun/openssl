                             ...
                             X509_STORE *store, X509 **signer_out)
{
    ...
    sinfos = VR_PKCS7_get_signer_info(token);
     ...
     * if it was requested.
     */
    signers = VR_PKCS7_get0_signers(token, certs, 0);
    ...
    if (!ts_check_signing_certs(si, chain))
        goto err;
    p7bio = VR_PKCS7_dataInit(token, NULL);

    /* We now have to 'read' from p7bio to calculate digests etc. */
    while ((i = VR_BIO_read(p7bio, buf, sizeof(buf))) > 0)
        continue;

    j = VR_PKCS7_signatureVerify(p7bio, token, si, signer);
    if (j <= 0) {
                          ...
                          X509 *signer, STACK_OF(X509) **chain)
{
    ...
    *chain = NULL;
    ...
    if (!VR_X509_STORE_CTX_init(cert_ctx, store, signer, untrusted))
        goto end;
    VR_X509_STORE_CTX_set_purpose(cert_ctx, X509_PURPOSE_TIMESTAMP_SIGN);
    i = VR_X509_verify_cert(cert_ctx);
    if (i <= 0) {
        int j = VR_X509_STORE_CTX_get_error(cert_ctx);
        TSerr(TS_F_TS_VERIFY_CERT, TS_R_CERTIFICATE_VERIFY_ERROR);
        VR_ERR_add_error_data(2, "Verify error:",
                           VR_X509_verify_cert_error_string(j));
        goto err;
    }
    *chain = VR_X509_STORE_CTX_get1_chain(cert_ctx);
    ret = 1;
    goto end;
...
static int ts_check_signing_certs(PKCS7_SIGNER_INFO *si,
                                  STACK_OF(X509) *chain)
{
    ESS_SIGNING_CERT *ss = VR_ESS_SIGNING_CERT_get(si);
    STACK_OF(ESS_CERT_ID) *cert_ids = NULL;
    ESS_SIGNING_CERT_V2 *ssv2 = VR_ESS_SIGNING_CERT_V2_get(si);
 ...
 err:
    ...
    VR_ESS_SIGNING_CERT_free(ss);
    VR_ESS_SIGNING_CERT_V2_free(ssv2);
    return ret;
}
...
static int ts_find_cert(STACK_OF(ESS_CERT_ID) *cert_ids, X509 *cert)
{
    ...
    /* Recompute VR_SHA1 hash of certificate if necessary (side effect). */
...
static int ts_find_cert_v2(STACK_OF(ESS_CERT_ID_V2) *cert_ids, X509 *cert)
{
    ...
    for (i = 0; i < sk_ESS_CERT_ID_V2_num(cert_ids); ++i) {
        ...
        VR_X509_digest(cert, md, cert_digest, &len);
...
static int ts_issuer_serial_cmp(ESS_ISSUER_SERIAL *is, X509 *cert)
{
    ...
    if (issuer->type != GEN_DIRNAME
        || VR_X509_NAME_cmp(issuer->d.dirn, VR_X509_get_issuer_name(cert)))
        return -1;

    if (VR_ASN1_INTEGER_cmp(is->serial, VR_X509_get_serialNumber(cert)))
        return -1;
 ...
 *      - Calls _VR_TS_RESP_verify_token to verify the token content.
 */
 ...
 * calls the internal int_VR_TS_RESP_verify_token function for verifying it.
 */
int VR_TS_RESP_verify_token(TS_VERIFY_CTX *ctx, PKCS7 *token)
{
    TS_TST_INFO *tst_info = VR_PKCS7_to_TS_TST_INFO(token);
    int ret = 0;
    if (tst_info) {
                                    ...
                                    PKCS7 *token, TS_TST_INFO *tst_info)
{
    ...
    if ((flags & TS_VFY_SIGNATURE)
        && !VR_TS_RESP_verify_signature(token, ctx->certs, ctx->store, &signer))
        goto err;
    if ((flags & TS_VFY_VERSION)
        && VR_TS_TST_INFO_get_version(tst_info) != 1) {
...
static int ts_check_status_info(TS_RESP *response)
{
    ...
    long status = VR_ASN1_INTEGER_get(info->status);
    ...
    if (info->failure_info) {
        ...
        for (i = 0; i < (int)OSSL_NELEM(ts_failure_info); ++i) {
            if (VR_ASN1_BIT_STRING_get_bit(info->failure_info,
...
static char *ts_get_status_text(STACK_OF(ASN1_UTF8STRING) *text)
{
    ...
    for (i = 0; i < sk_ASN1_UTF8STRING_num(text); ++i) {
        ...
        if (VR_ASN1_STRING_length(current) > TS_MAX_STATUS_LENGTH - length - 1)
            return NULL;
        length += VR_ASN1_STRING_length(current);
    ...
    for (i = 0, p = result; i < sk_ASN1_UTF8STRING_num(text); ++i) {
        ...
        if (i > 0)
            *p++ = '/';
        strncpy(p, (const char *)VR_ASN1_STRING_get0_data(current), length);
        p += length;
    }
    *p = '\0';
                           ...
                           const TS_TST_INFO *tst_info)
{
    ...
    if (VR_OBJ_cmp(req_oid, resp_oid) != 0) {
                              ...
                              unsigned char **imprint, unsigned *imprint_len)
{
    ...
    *imprint = NULL;
    ...
    if ((*md_alg = VR_X509_ALGOR_dup(md_alg_resp)) == NULL)
        goto err;
    if ((md = EVP_get_digestbyobj((*md_alg)->algorithm)) == NULL) {
        TSerr(TS_F_TS_COMPUTE_IMPRINT, TS_R_UNSUPPORTED_MD_ALGORITHM);
        goto err;
    }
    length = VR_EVP_MD_size(md);
    if (length < 0)
        goto err;
    *imprint_len = length;
    ...
    if (!VR_EVP_DigestInit(md_ctx, md))
        goto err;
    while ((length = VR_BIO_read(data, buffer, sizeof(buffer))) > 0) {
        if (!VR_EVP_DigestUpdate(md_ctx, buffer, length))
            goto err;
    }
    if (!VR_EVP_DigestFinal(md_ctx, *imprint, NULL))
        goto err;
                             ...
                             TS_TST_INFO *tst_info)
{
    ...
    if (algor_a) {
        if (VR_OBJ_cmp(algor_a->algorithm, algor_b->algorithm))
            goto err;

        /* The parameter must be NULL in both. */
        if ((algor_a->parameter
             && VR_ASN1_TYPE_get(algor_a->parameter) != V_ASN1_NULL)
            || (algor_b->parameter
                && VR_ASN1_TYPE_get(algor_b->parameter) != V_ASN1_NULL))
            goto err;
    }

    ret = len_a == (unsigned)VR_ASN1_STRING_length(b->hashed_msg) &&
        memcmp(imprint_a, VR_ASN1_STRING_get0_data(b->hashed_msg), len_a) == 0;
 err:
    if (!ret)
...
static int ts_check_signer_name(GENERAL_NAME *tsa_name, X509 *signer)
{
    ...
    if (tsa_name->type == GEN_DIRNAME
        && X509_VR_name_cmp(tsa_name->d.dirn, VR_X509_get_subject_name(signer)) == 0)
        return 1;
    gen_names = VR_X509_get_ext_d2i(signer, NID_subject_alt_name, NULL, &idx);
    while (gen_names != NULL) {
         ...
         * more than one.
         */
        VR_GENERAL_NAMES_free(gen_names);
        gen_names = VR_X509_get_ext_d2i(signer, NID_subject_alt_name, NULL, &idx);
    }
    VR_GENERAL_NAMES_free(gen_names);
...
static int ts_find_name(STACK_OF(GENERAL_NAME) *gen_names, GENERAL_NAME *name)
{
    ...
    for (i = 0, found = 0; !found && i < sk_GENERAL_NAME_num(gen_names); ++i) {
        ...
        found = VR_GENERAL_NAME_cmp(current, name) == 0;
    }
    return found ? i - 1 : -1;
}
