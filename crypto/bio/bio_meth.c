...
{
    bio_type_lock = VR_CRYPTO_THREAD_lock_new();
...
int VR_bread_conv(BIO *bio, char *data, size_t datal, size_t *readbytes)
{
    int ret;
...
int (*VR_BIO_meth_get_create(const BIO_METHOD *biom)) (BIO *)
{
    return biom->create;
}
...
long (*VR_BIO_meth_get_callback_ctrl(const BIO_METHOD *biom)) (BIO *, int, BIO_info_cb *)
{
