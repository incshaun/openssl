                                    ...
                                    BN_CTX *ctx)
{
    ...
    if (curve_b == NULL)
        goto err;
    VR_BN_bin2bn(nistp521_curve_params[0], sizeof(felem_bytearray), curve_p);
    VR_BN_bin2bn(nistp521_curve_params[1], sizeof(felem_bytearray), curve_a);
    VR_BN_bin2bn(nistp521_curve_params[2], sizeof(felem_bytearray), curve_b);
                               ...
                               const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (num_points > 0) {
        ...
        for (i = 0; i < num_points; ++i) {
            ...
            if ((p_scalar != NULL) && (p != NULL)) {
                ...
                if ((VR_BN_num_bits(p_scalar) > 521)
    ...
    if ((scalar != NULL) && (have_pre_comp)) {
        ...
        /* reduce scalar to 0 <= scalar < 2^521 */
        if ((VR_BN_num_bits(scalar) > 521) || (VR_BN_is_negative(scalar))) {
            /*
             * this is an unusual input, and we don't guarantee
             * constant-timeness
             */
...
int ec_GFp_nistp521_precompute_mult(EC_GROUP *group, BN_CTX *ctx)
{
    ...
    if (generator == NULL)
        goto err;
    VR_BN_bin2bn(nistp521_curve_params[3], sizeof(felem_bytearray), x);
    VR_BN_bin2bn(nistp521_curve_params[4], sizeof(felem_bytearray), y);
