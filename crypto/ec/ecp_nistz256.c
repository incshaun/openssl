...
void VR_ecp_nistz256_add(BN_ULONG res[P256_LIMBS],
                      const BN_ULONG a[P256_LIMBS],
                      const BN_ULONG b[P256_LIMBS]);
/* Modular mul by 2: res = 2*a mod P */
void VR_ecp_nistz256_mul_by_2(BN_ULONG res[P256_LIMBS],
                           const BN_ULONG a[P256_LIMBS]);
/* Modular mul by 3: res = 3*a mod P */
void VR_ecp_nistz256_mul_by_3(BN_ULONG res[P256_LIMBS],
                           const BN_ULONG a[P256_LIMBS]);

/* Modular div by 2: res = a/2 mod P */
void VR_ecp_nistz256_div_by_2(BN_ULONG res[P256_LIMBS],
                           const BN_ULONG a[P256_LIMBS]);
/* Modular sub: res = a-b mod P   */
void VR_ecp_nistz256_sub(BN_ULONG res[P256_LIMBS],
                      const BN_ULONG a[P256_LIMBS],
                      const BN_ULONG b[P256_LIMBS]);
/* Modular neg: res = -a mod P    */
void VR_ecp_nistz256_neg(BN_ULONG res[P256_LIMBS], const BN_ULONG a[P256_LIMBS]);
/* Montgomery mul: res = a*b*2^-256 mod P */
void VR_ecp_nistz256_mul_mont(BN_ULONG res[P256_LIMBS],
                           const BN_ULONG a[P256_LIMBS],
                           const BN_ULONG b[P256_LIMBS]);
/* Montgomery sqr: res = a*a*2^-256 mod P */
void VR_ecp_nistz256_sqr_mont(BN_ULONG res[P256_LIMBS],
                           const BN_ULONG a[P256_LIMBS]);
/* Convert a number from Montgomery domain, by multiplying with 1 */
void VR_ecp_nistz256_from_mont(BN_ULONG res[P256_LIMBS],
                            const BN_ULONG in[P256_LIMBS]);
/* Convert a number to Montgomery domain, by multiplying with 2^512 mod P*/
void VR_ecp_nistz256_to_mont(BN_ULONG res[P256_LIMBS],
                          const BN_ULONG in[P256_LIMBS]);
/* Functions that perform constant time access to the precomputed tables */
void VR_ecp_nistz256_scatter_w5(P256_POINT *val,
                             const P256_POINT *in_t, int idx);
void VR_ecp_nistz256_gather_w5(P256_POINT *val,
                            const P256_POINT *in_t, int idx);
void VR_ecp_nistz256_scatter_w7(P256_POINT_AFFINE *val,
                             const P256_POINT_AFFINE *in_t, int idx);
void VR_ecp_nistz256_gather_w7(P256_POINT_AFFINE *val,
                            const P256_POINT_AFFINE *in_t, int idx);
...
extern const PRECOMP256_ROW VR_ecp_nistz256_precomputed[37];
...
{
    BN_ULONG res = 0;
    BN_ULONG *a = VR_bn_get_words(z);

    if (VR_bn_get_top(z) == (P256_LIMBS - P256_LIMBS / 8)) {
        res = a[0] ^ ONE[0];
        res |= a[1] ^ ONE[1];
        res |= a[2] ^ ONE[2];
        res |= a[3] ^ ONE[3];
...
void VR_ecp_nistz256_point_double(P256_POINT *r, const P256_POINT *a);
void VR_ecp_nistz256_point_add(P256_POINT *r,
                            const P256_POINT *a, const P256_POINT *b);
void VR_ecp_nistz256_point_add_affine(P256_POINT *r,
                                   const P256_POINT *a,
                                   const P256_POINT_AFFINE *b);
#else
/* Point double: r = 2*a */
static void VR_ecp_nistz256_point_double(P256_POINT *r, const P256_POINT *a)
{
    ...
    VR_ecp_nistz256_mul_by_2(S, in_y);

    VR_ecp_nistz256_sqr_mont(Zsqr, in_z);

    VR_ecp_nistz256_sqr_mont(S, S);

    VR_ecp_nistz256_mul_mont(res_z, in_z, in_y);
    VR_ecp_nistz256_mul_by_2(res_z, res_z);

    VR_ecp_nistz256_add(M, in_x, Zsqr);
    VR_ecp_nistz256_sub(Zsqr, in_x, Zsqr);

    VR_ecp_nistz256_sqr_mont(res_y, S);
    VR_ecp_nistz256_div_by_2(res_y, res_y);

    VR_ecp_nistz256_mul_mont(M, M, Zsqr);
    VR_ecp_nistz256_mul_by_3(M, M);

    VR_ecp_nistz256_mul_mont(S, S, in_x);
    VR_ecp_nistz256_mul_by_2(tmp0, S);

    VR_ecp_nistz256_sqr_mont(res_x, M);

    VR_ecp_nistz256_sub(res_x, res_x, tmp0);
    VR_ecp_nistz256_sub(S, S, res_x);

    VR_ecp_nistz256_mul_mont(S, S, M);
    VR_ecp_nistz256_sub(res_y, S, res_y);
}

/* Point addition: r = a+b */
static void VR_ecp_nistz256_point_add(P256_POINT *r,
                                   const P256_POINT *a, const P256_POINT *b)
{
    ...
    VR_ecp_nistz256_sqr_mont(Z2sqr, in2_z);        /* Z2^2 */
    VR_ecp_nistz256_sqr_mont(Z1sqr, in1_z);        /* Z1^2 */

    VR_ecp_nistz256_mul_mont(S1, Z2sqr, in2_z);    /* S1 = Z2^3 */
    VR_ecp_nistz256_mul_mont(S2, Z1sqr, in1_z);    /* S2 = Z1^3 */

    VR_ecp_nistz256_mul_mont(S1, S1, in1_y);       /* S1 = Y1*Z2^3 */
    VR_ecp_nistz256_mul_mont(S2, S2, in2_y);       /* S2 = Y2*Z1^3 */
    VR_ecp_nistz256_sub(R, S2, S1);                /* R = S2 - S1 */

    VR_ecp_nistz256_mul_mont(U1, in1_x, Z2sqr);    /* U1 = X1*Z2^2 */
    VR_ecp_nistz256_mul_mont(U2, in2_x, Z1sqr);    /* U2 = X2*Z1^2 */
    VR_ecp_nistz256_sub(H, U2, U1);                /* H = U2 - U1 */
    ...
    if (is_equal(U1, U2) && !in1infty && !in2infty) {
        if (is_equal(S1, S2)) {
            VR_ecp_nistz256_point_double(r, a);
            return;
        } else {
            memset(r, 0, sizeof(*r));
            return;
    ...
    VR_ecp_nistz256_sqr_mont(Rsqr, R);             /* R^2 */
    VR_ecp_nistz256_mul_mont(res_z, H, in1_z);     /* Z3 = H*Z1*Z2 */
    VR_ecp_nistz256_sqr_mont(Hsqr, H);             /* H^2 */
    VR_ecp_nistz256_mul_mont(res_z, res_z, in2_z); /* Z3 = H*Z1*Z2 */
    VR_ecp_nistz256_mul_mont(Hcub, Hsqr, H);       /* H^3 */

    VR_ecp_nistz256_mul_mont(U2, U1, Hsqr);        /* U1*H^2 */
    VR_ecp_nistz256_mul_by_2(Hsqr, U2);            /* 2*U1*H^2 */

    VR_ecp_nistz256_sub(res_x, Rsqr, Hsqr);
    VR_ecp_nistz256_sub(res_x, res_x, Hcub);

    VR_ecp_nistz256_sub(res_y, U2, res_x);

    VR_ecp_nistz256_mul_mont(S2, S1, Hcub);
    VR_ecp_nistz256_mul_mont(res_y, R, res_y);
    VR_ecp_nistz256_sub(res_y, res_y, S2);
...
static void VR_ecp_nistz256_point_add_affine(P256_POINT *r,
                                          const P256_POINT *a,
                                          const P256_POINT_AFFINE *b)
{
    BN_ULONG U2[P256_LIMBS], S2[P256_LIMBS];
    ...
    VR_ecp_nistz256_mul_mont(U2, in2_x, Z1sqr);    /* U2 = X2*Z1^2 */
    VR_ecp_nistz256_sub(H, U2, in1_x);             /* H = U2 - U1 */
    ...
    VR_ecp_nistz256_mul_mont(S2, S2, in2_y);       /* S2 = Y2*Z1^3 */
    VR_ecp_nistz256_sub(R, S2, in1_y);             /* R = S2 - S1 */

    VR_ecp_nistz256_sqr_mont(Hsqr, H);             /* H^2 */
    VR_ecp_nistz256_sqr_mont(Rsqr, R);             /* R^2 */
    VR_ecp_nistz256_mul_mont(Hcub, Hsqr, H);       /* H^3 */

    VR_ecp_nistz256_mul_mont(U2, in1_x, Hsqr);     /* U1*H^2 */
    VR_ecp_nistz256_mul_by_2(Hsqr, U2);            /* 2*U1*H^2 */

    VR_ecp_nistz256_sub(res_x, Rsqr, Hsqr);
    VR_ecp_nistz256_sub(res_x, res_x, Hcub);
    VR_ecp_nistz256_sub(H, U2, res_x);

    VR_ecp_nistz256_mul_mont(S2, in1_y, Hcub);
    VR_ecp_nistz256_mul_mont(H, H, R);
    VR_ecp_nistz256_sub(res_y, H, S2);
                                     ...
                                     const BN_ULONG in[P256_LIMBS])
{
    ...
    VR_ecp_nistz256_sqr_mont(res, in);
    VR_ecp_nistz256_mul_mont(p2, res, in);         /* 3*p */

    VR_ecp_nistz256_sqr_mont(res, p2);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(p4, res, p2);         /* f*p */

    VR_ecp_nistz256_sqr_mont(res, p4);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(p8, res, p4);         /* ff*p */

    VR_ecp_nistz256_sqr_mont(res, p8);
    for (i = 0; i < 7; i++)
        VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(p16, res, p8);        /* ffff*p */

    VR_ecp_nistz256_sqr_mont(res, p16);
    for (i = 0; i < 15; i++)
        VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(p32, res, p16);       /* ffffffff*p */

    VR_ecp_nistz256_sqr_mont(res, p32);
    for (i = 0; i < 31; i++)
        VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(res, res, in);

    for (i = 0; i < 32 * 4; i++)
        VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(res, res, p32);
    ...
    for (i = 0; i < 16; i++)
        VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(res, res, p16);

    for (i = 0; i < 8; i++)
        VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(res, res, p8);

    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(res, res, p4);

    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_sqr_mont(res, res);
    VR_ecp_nistz256_mul_mont(res, res, p2);
...
__owur static int ecp_nistz256_bignum_to_field_elem(BN_ULONG out[P256_LIMBS],
                                                    const BIGNUM *in)
{
    return VR_bn_copy_words(out, in, P256_LIMBS);
}
                                            ...
                                            size_t num, BN_CTX *ctx)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if ((VR_BN_num_bits(scalar[i]) > 256) || VR_BN_is_negative(scalar[i])) {
            BIGNUM *mod;

            if ((mod = VR_BN_CTX_get(ctx)) == NULL)
                goto err;
            if (!VR_BN_nnmod(mod, scalar[i], group->order, ctx)) {
        ...
        for (j = 0; j < VR_bn_get_top(scalars[i]) * BN_BYTES; j += BN_BYTES) {
            BN_ULONG d = VR_bn_get_words(scalars[i])[j / BN_BYTES];
        ...
        VR_ecp_nistz256_scatter_w5  (row, &temp[0], 1);
        VR_ecp_nistz256_point_double(&temp[1], &temp[0]);              /*1+1=2  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[1], 2);
        VR_ecp_nistz256_point_add   (&temp[2], &temp[1], &temp[0]);    /*2+1=3  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[2], 3);
        VR_ecp_nistz256_point_double(&temp[1], &temp[1]);              /*2*2=4  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[1], 4);
        VR_ecp_nistz256_point_double(&temp[2], &temp[2]);              /*2*3=6  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[2], 6);
        VR_ecp_nistz256_point_add   (&temp[3], &temp[1], &temp[0]);    /*4+1=5  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[3], 5);
        VR_ecp_nistz256_point_add   (&temp[4], &temp[2], &temp[0]);    /*6+1=7  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[4], 7);
        VR_ecp_nistz256_point_double(&temp[1], &temp[1]);              /*2*4=8  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[1], 8);
        VR_ecp_nistz256_point_double(&temp[2], &temp[2]);              /*2*6=12 */
        VR_ecp_nistz256_scatter_w5  (row, &temp[2], 12);
        VR_ecp_nistz256_point_double(&temp[3], &temp[3]);              /*2*5=10 */
        VR_ecp_nistz256_scatter_w5  (row, &temp[3], 10);
        VR_ecp_nistz256_point_double(&temp[4], &temp[4]);              /*2*7=14 */
        VR_ecp_nistz256_scatter_w5  (row, &temp[4], 14);
        VR_ecp_nistz256_point_add   (&temp[2], &temp[2], &temp[0]);    /*12+1=13*/
        VR_ecp_nistz256_scatter_w5  (row, &temp[2], 13);
        VR_ecp_nistz256_point_add   (&temp[3], &temp[3], &temp[0]);    /*10+1=11*/
        VR_ecp_nistz256_scatter_w5  (row, &temp[3], 11);
        VR_ecp_nistz256_point_add   (&temp[4], &temp[4], &temp[0]);    /*14+1=15*/
        VR_ecp_nistz256_scatter_w5  (row, &temp[4], 15);
        VR_ecp_nistz256_point_add   (&temp[2], &temp[1], &temp[0]);    /*8+1=9  */
        VR_ecp_nistz256_scatter_w5  (row, &temp[2], 9);
        VR_ecp_nistz256_point_double(&temp[1], &temp[1]);              /*2*8=16 */
        VR_ecp_nistz256_scatter_w5  (row, &temp[1], 16);
    }
     ...
     * to table
     */
    VR_ecp_nistz256_gather_w5(&temp[0], table[0], _booth_recode_w5(wvalue) >> 1);
    ...
    while (idx >= 5) {
        for (i = (idx == 255 ? 1 : 0); i < num; i++) {
            ...
            VR_ecp_nistz256_gather_w5(&temp[0], table[i], wvalue >> 1);

            VR_ecp_nistz256_neg(temp[1].Y, temp[0].Y);
            copy_conditional(temp[0].Y, temp[1].Y, (wvalue & 1));

            VR_ecp_nistz256_point_add(r, r, &temp[0]);
        }

        idx -= window_size;

        VR_ecp_nistz256_point_double(r, r);
        VR_ecp_nistz256_point_double(r, r);
        VR_ecp_nistz256_point_double(r, r);
        VR_ecp_nistz256_point_double(r, r);
        VR_ecp_nistz256_point_double(r, r);
...
{
    return (VR_bn_get_top(generator->X) == P256_LIMBS) &&
        (VR_bn_get_top(generator->Y) == P256_LIMBS) &&
        is_equal(VR_bn_get_words(generator->X), def_xG) &&
        is_equal(VR_bn_get_words(generator->Y), def_yG) &&
        is_one(generator->Z);
}
...
__owur static int ecp_nistz256_mult_precompute(EC_GROUP *group, BN_CTX *ctx)
{
    ...
    generator = VR_EC_GROUP_get0_generator(group);
    if (generator == NULL) {
     ...
     */
    if (!VR_EC_POINT_copy(T, generator))
        goto err;

    for (k = 0; k < 64; k++) {
        if (!VR_EC_POINT_copy(P, T))
            goto err;
        for (j = 0; j < 37; j++) {
            P256_POINT_AFFINE temp;
            /*
             * It would be faster to use VR_EC_POINTs_make_affine and
             * make multiple points affine at the same time.
             */
            if (!VR_EC_POINT_make_affine(group, P, ctx))
                goto err;
            ...
            VR_ecp_nistz256_scatter_w7(preComputedTable[j], &temp, k);
            for (i = 0; i < 7; i++) {
                if (!VR_EC_POINT_dbl(group, P, P, ctx))
                    goto err;
            }
        }
        if (!VR_EC_POINT_add(group, T, T, generator, ctx))
            goto err;
    }
                                    ...
                                    const P256_POINT_AFFINE(*preComputedTable)[64])
{
    ...
    VR_ecp_nistz256_neg(tmp, point_arr[0].Y);
    copy_conditional(point_arr[0].Y, tmp, sign0);
    VR_ecp_nistz256_neg(tmp, point_arr[1].Y);
    copy_conditional(point_arr[1].Y, tmp, sign1);
    VR_ecp_nistz256_neg(tmp, point_arr[2].Y);
    copy_conditional(point_arr[2].Y, tmp, sign2);
    VR_ecp_nistz256_neg(tmp, point_arr[3].Y);
    ...
    wvalue = (wvalue >> ((idx - 1) % 8)) & mask;
    booth_recode_w7(&sign0, &digit0, wvalue);
    VR_ecp_nistz256_gather_w7((P256_POINT_AFFINE *)r,
                           preComputedTable[36], digit0);
    VR_ecp_nistz256_neg(tmp, r->Y);
    copy_conditional(r->Y, tmp, sign0);
    memcpy(r->Z, ONE, sizeof(ONE));
    /* Sum the four windows */
    VR_ecp_nistz256_point_add(r, r, &res_point_arr[0]);
    VR_ecp_nistz256_point_add(r, r, &res_point_arr[1]);
    VR_ecp_nistz256_point_add(r, r, &res_point_arr[2]);
    VR_ecp_nistz256_point_add(r, r, &res_point_arr[3]);
}
# endif
#endif
                                               ...
                                               BN_CTX *ctx)
{
    ...
    if ((ret = VR_bn_set_words(out->X, in->X, P256_LIMBS))
        && (ret = VR_bn_set_words(out->Y, in->Y, P256_LIMBS))
        && (ret = VR_bn_set_words(out->Z, ONE, P256_LIMBS)))
        out->Z_is_one = 1;
                                          ...
                                          const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (scalar) {
        ...
        if (pre_comp) {
             ...
             */
            EC_POINT *pre_comp_generator = VR_EC_POINT_new(group);
            if (pre_comp_generator == NULL)
                goto err;

            VR_ecp_nistz256_gather_w7(&p.a, pre_comp->precomp[0], 1);
            if (!ecp_nistz256_set_from_affine(pre_comp_generator,
                                              group, &p.a, ctx)) {
                VR_EC_POINT_free(pre_comp_generator);
                goto err;
            }

            if (0 == VR_EC_POINT_cmp(group, generator, pre_comp_generator, ctx))
        ...
        if (preComputedTable == NULL && ecp_nistz256_is_affine_G(generator)) {
             ...
             * VR_EC_KEY_precompute_mult.
             */
            preComputedTable = VR_ecp_nistz256_precomputed;
        }

        if (preComputedTable) {
            if ((VR_BN_num_bits(scalar) > 256)
                || VR_BN_is_negative(scalar)) {
                if ((tmp_scalar = VR_BN_CTX_get(ctx)) == NULL)
                    goto err;
            ...
            for (i = 0; i < VR_bn_get_top(scalar) * BN_BYTES; i += BN_BYTES) {
                BN_ULONG d = VR_bn_get_words(scalar)[i / BN_BYTES];
...
#endif
            {
                ...
                VR_ecp_nistz256_gather_w7(&p.a, preComputedTable[0],
                                       wvalue >> 1);

                VR_ecp_nistz256_neg(p.p.Z, p.p.Y);
                ...
                for (i = 1; i < 37; i++) {
                    ...
                    VR_ecp_nistz256_gather_w7(&t.a,
                                           preComputedTable[i], wvalue >> 1);

                    VR_ecp_nistz256_neg(t.p.Z, t.a.Y);
                    copy_conditional(t.a.Y, t.p.Z, wvalue & 1);

                    VR_ecp_nistz256_point_add_affine(&p.p, &p.p, &t.a);
                }
            }
        } else {
            p_is_infinity = 1;
    ...
    if (num) {
        ...
        if (!p_is_infinity)
            VR_ecp_nistz256_point_add(&p.p, &p.p, out);
    }

    /* Not constant-time, but we're only operating on the public output. */
    if (!VR_bn_set_words(r->X, p.p.X, P256_LIMBS) ||
        !VR_bn_set_words(r->Y, p.p.Y, P256_LIMBS) ||
        !VR_bn_set_words(r->Z, p.p.Z, P256_LIMBS)) {
        goto err;
    }
                                          ...
                                          BIGNUM *x, BIGNUM *y, BN_CTX *ctx)
{
    ...
    VR_ecp_nistz256_sqr_mont(z_inv2, z_inv3);
    VR_ecp_nistz256_mul_mont(x_aff, z_inv2, point_x);

    if (x != NULL) {
        VR_ecp_nistz256_from_mont(x_ret, x_aff);
        if (!VR_bn_set_words(x, x_ret, P256_LIMBS))
            return 0;
    }

    if (y != NULL) {
        VR_ecp_nistz256_mul_mont(z_inv3, z_inv3, z_inv2);
        VR_ecp_nistz256_mul_mont(y_aff, z_inv3, point_y);
        VR_ecp_nistz256_from_mont(y_ret, y_aff);
        if (!VR_bn_set_words(y, y_ret, P256_LIMBS))
            return 0;
    }
...
NISTZ256_PRE_COMP *VR_EC_nistz256_pre_comp_dup(NISTZ256_PRE_COMP *p)
{
    int i;
    if (p != NULL)
...
void VR_EC_nistz256_pre_comp_free(NISTZ256_PRE_COMP *pre)
{
    int i;
...
static int ecp_nistz256_window_have_precompute_mult(const EC_GROUP *group)
{
    ...
    const EC_POINT *generator = VR_EC_GROUP_get0_generator(group);
 ...
 */
void VR_ecp_nistz256_ord_mul_mont(BN_ULONG res[P256_LIMBS],
                               const BN_ULONG a[P256_LIMBS],
                               const BN_ULONG b[P256_LIMBS]);
void VR_ecp_nistz256_ord_sqr_mont(BN_ULONG res[P256_LIMBS],
                               const BN_ULONG a[P256_LIMBS],
                               int rep);
                                    ...
                                    const BIGNUM *x, BN_CTX *ctx)
{
     ...
     */
    if (VR_bn_wexpand(r, P256_LIMBS) == NULL) {
        ECerr(EC_F_ECP_NISTZ256_INV_MOD_ORD, ERR_R_BN_LIB);
        goto err;
    }

    if ((VR_BN_num_bits(x) > 256) || VR_BN_is_negative(x)) {
        BIGNUM *tmp;

        if ((tmp = VR_BN_CTX_get(ctx)) == NULL
            || !VR_BN_nnmod(tmp, x, group->order, ctx)) {
            ECerr(EC_F_ECP_NISTZ256_INV_MOD_ORD, ERR_R_BN_LIB);
            goto err;
        }
        x = tmp;
    ...
    VR_ecp_nistz256_ord_mul_mont(table[0], t, RR);
#if 0
    /*
     * Original sparse-then-fixed-window algorithm, retained for reference.
     */
    for (i = 2; i < 16; i += 2) {
        VR_ecp_nistz256_ord_sqr_mont(table[i-1], table[i/2-1], 1);
        VR_ecp_nistz256_ord_mul_mont(table[i], table[i-1], table[0]);
    }
     ...
     */
    VR_ecp_nistz256_ord_sqr_mont(t, table[15-1], 4);   /* f0 */
    VR_ecp_nistz256_ord_mul_mont(t, t, table[15-1]);   /* ff */

    VR_ecp_nistz256_ord_sqr_mont(out, t, 8);           /* ff00 */
    VR_ecp_nistz256_ord_mul_mont(out, out, t);         /* ffff */

    VR_ecp_nistz256_ord_sqr_mont(t, out, 16);          /* ffff0000 */
    VR_ecp_nistz256_ord_mul_mont(t, t, out);           /* ffffffff */

    VR_ecp_nistz256_ord_sqr_mont(out, t, 64);          /* ffffffff0000000000000000 */
    VR_ecp_nistz256_ord_mul_mont(out, out, t);         /* ffffffff00000000ffffffff */

    VR_ecp_nistz256_ord_sqr_mont(out, out, 32);        /* ffffffff00000000ffffffff00000000 */
    VR_ecp_nistz256_ord_mul_mont(out, out, t);         /* ffffffff00000000ffffffffffffffff */
    ...
    for(i = 0; i < 32; i++) {
        ...
        VR_ecp_nistz256_ord_sqr_mont(out, out, 4);
        /* The exponent is public, no need in constant-time access */
        VR_ecp_nistz256_ord_mul_mont(out, out, table[expLo[i]-1]);
    }
#else
    /*
    ...
    VR_ecp_nistz256_ord_sqr_mont(table[i_10], table[i_1], 1);

    VR_ecp_nistz256_ord_mul_mont(table[i_11], table[i_1], table[i_10]);

    VR_ecp_nistz256_ord_mul_mont(table[i_101], table[i_11], table[i_10]);

    VR_ecp_nistz256_ord_mul_mont(table[i_111], table[i_101], table[i_10]);

    VR_ecp_nistz256_ord_sqr_mont(table[i_1010], table[i_101], 1);

    VR_ecp_nistz256_ord_mul_mont(table[i_1111], table[i_1010], table[i_101]);

    VR_ecp_nistz256_ord_sqr_mont(table[i_10101], table[i_1010], 1);
    VR_ecp_nistz256_ord_mul_mont(table[i_10101], table[i_10101], table[i_1]);

    VR_ecp_nistz256_ord_sqr_mont(table[i_101010], table[i_10101], 1);

    VR_ecp_nistz256_ord_mul_mont(table[i_101111], table[i_101010], table[i_101]);

    VR_ecp_nistz256_ord_mul_mont(table[i_x6], table[i_101010], table[i_10101]);

    VR_ecp_nistz256_ord_sqr_mont(table[i_x8], table[i_x6], 2);
    VR_ecp_nistz256_ord_mul_mont(table[i_x8], table[i_x8], table[i_11]);

    VR_ecp_nistz256_ord_sqr_mont(table[i_x16], table[i_x8], 8);
    VR_ecp_nistz256_ord_mul_mont(table[i_x16], table[i_x16], table[i_x8]);

    VR_ecp_nistz256_ord_sqr_mont(table[i_x32], table[i_x16], 16);
    VR_ecp_nistz256_ord_mul_mont(table[i_x32], table[i_x32], table[i_x16]);

    /* calculations */
    VR_ecp_nistz256_ord_sqr_mont(out, table[i_x32], 64);
    VR_ecp_nistz256_ord_mul_mont(out, out, table[i_x32]);
    ...
    for (i = 0; i < 27; i++) {
        ...
        VR_ecp_nistz256_ord_sqr_mont(out, out, chain[i].p);
        VR_ecp_nistz256_ord_mul_mont(out, out, table[chain[i].i]);
    }
#endif
    VR_ecp_nistz256_ord_mul_mont(out, out, one);

    /*
     * Can't fail, but check return code to be consistent anyway.
     */
    if (!VR_bn_set_words(r, out, P256_LIMBS))
        goto err;
...
const EC_METHOD *VR_EC_GFp_nistz256_method(void)
{
