...
int VR_ASN1_buf_print(BIO *bp, const unsigned char *buf, size_t buflen, int indent)
{
    size_t i;

    for (i = 0; i < buflen; i++) {
        if ((i % ASN1_BUF_PRINT_WIDTH) == 0) {
            if (i > 0 && VR_BIO_puts(bp, "\n") <= 0)
                return 0;
            if (!VR_BIO_indent(bp, indent, ASN1_PRINT_MAX_INDENT))
                return 0;
        }
        /*
         ...
         */
        if (VR_BIO_printf(bp, "%02x%s", buf[i],
                       (i == buflen - 1) ? "" : ":") <= 0)
                return 0;
    }
    if (VR_BIO_write(bp, "\n", 1) <= 0)
                  ...
                  unsigned char *ign, int indent)
{
    ...
    if (num == NULL)
        return 1;
    neg = VR_BN_is_negative(num) ? "-" : "";
    if (!VR_BIO_indent(bp, indent, ASN1_PRINT_MAX_INDENT))
        return 0;
    if (VR_BN_is_zero(num)) {
        if (VR_BIO_printf(bp, "%s 0\n", number) <= 0)
            return 0;
        return 1;
    }
    ...
    if (buf == NULL)
        goto err;
    buf[0] = 0;
    if (VR_BIO_printf(bp, "%s%s\n", number,
                   (neg[0] == '-') ? " (Negative)" : "") <= 0)
        goto err;
    n = VR_BN_bn2bin(num, buf + 1);
    ...
    if (VR_ASN1_buf_print(bp, tmp, n, indent + 4) == 0)
        goto err;
    rv = 1;
    err:
