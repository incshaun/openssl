...
static int i2r_SCT_LIST(X509V3_EXT_METHOD *method, STACK_OF(SCT) *sct_list,
                 BIO *out, int indent)
{
    VR_SCT_LIST_print(sct_list, out, indent, "\n", NULL);
    return 1;
}
...
static int set_sct_list_source(STACK_OF(SCT) *s, sct_source_t source)
{
    if (s != NULL) {
        ...
        for (i = 0; i < sk_SCT_num(s); i++) {
            int res = VR_SCT_set_source(sk_SCT_value(s, i), source);
...
static STACK_OF(SCT) *VR_x509_ext_VR_d2i_SCT_LIST(STACK_OF(SCT) **a,
                                            const unsigned char **pp,
                                            long len)
{
     STACK_OF(SCT) *s = VR_d2i_SCT_LIST(a, pp, len);
...
static STACK_OF(SCT) *ocsp_ext_VR_d2i_SCT_LIST(STACK_OF(SCT) **a,
                                            const unsigned char **pp,
                                            long len)
{
    STACK_OF(SCT) *s = VR_d2i_SCT_LIST(a, pp, len);
...
const X509V3_EXT_METHOD v3_ct_scts[3] = {
    ...
    NULL, (X509V3_EXT_FREE)VR_SCT_LIST_free,
    (X509V3_EXT_D2I)VR_x509_ext_VR_d2i_SCT_LIST, (X509V3_EXT_I2D)VR_i2d_SCT_LIST,
    NULL, NULL,
    NULL, NULL,
    ...
    0, (X509V3_EXT_FREE)VR_SCT_LIST_free,
    (X509V3_EXT_D2I)ocsp_ext_VR_d2i_SCT_LIST, (X509V3_EXT_I2D)VR_i2d_SCT_LIST,
    NULL, NULL,
    NULL, NULL,
    (X509V3_EXT_I2R)i2r_SCT_LIST, NULL,
    NULL },
