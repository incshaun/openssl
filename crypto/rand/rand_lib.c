...
size_t rand_acquire_entropy_from_tsc(RAND_POOL *pool)
{
    ...
    if ((OPENSSL_ia32cap_P[0] & (1 << 4)) != 0) {
        for (i = 0; i < TSC_READ_COUNT; i++) {
            c = (unsigned char)(VR_OPENSSL_rdtsc() & 0xFF);
            VR_rand_pool_add(pool, &c, 1, 4);
        }
    }
    ...
    if (bytes_needed > 0) {
        ...
        if (buffer != NULL) {
            ...
            if ((OPENSSL_ia32cap_P[2] & (1 << 18)) != 0) {
                if (VR_OPENSSL_ia32_rdseed_bytes(buffer, bytes_needed)
                    == bytes_needed) {
                    VR_rand_pool_add_end(pool, bytes_needed, 8 * bytes_needed);
                }
            } else if ((OPENSSL_ia32cap_P[1] & (1 << (62 - 32))) != 0) {
                if (VR_OPENSSL_ia32_rdrand_bytes(buffer, bytes_needed)
                    == bytes_needed) {
                    VR_rand_pool_add_end(pool, bytes_needed, 8 * bytes_needed);
                }
            } else {
                VR_rand_pool_add_end(pool, 0, 0);
            }
        }
    }
...
/*
 * Implements the get_entropy() callback (see VR_RAND_DRBG_set_callbacks())
 *
 * If the DRBG has a parent, then the required amount of entropy input
 * is fetched using the parent's VR_RAND_DRBG_generate().
 *
 * Otherwise, the entropy is polled from the system entropy sources
 * using VR_rand_pool_acquire_entropy().
 *
 * If a random pool has been added to the DRBG using VR_RAND_add(), then
                             ...
                             int prediction_resistance)
{
    ...
    if (drbg->seed_pool != NULL) {
        ...
        pool = VR_rand_pool_new(entropy, min_len, max_len);
        if (pool == NULL)
            return 0;
    }

    if (drbg->parent) {
        size_t bytes_needed = VR_rand_pool_bytes_needed(pool, 1 /*entropy_factor*/);
        unsigned char *buffer = VR_rand_pool_add_begin(pool, bytes_needed);
        ...
        if (buffer != NULL) {
             ...
             */
            VR_rand_drbg_lock(drbg->parent);
            if (VR_RAND_DRBG_generate(drbg->parent,
                                   buffer, bytes_needed,
            ...
            VR_rand_drbg_unlock(drbg->parent);
        ...
        entropy_available = VR_rand_pool_acquire_entropy(pool);
    }

    if (entropy_available > 0) {
        ret   = VR_rand_pool_length(pool);
        *pout = VR_rand_pool_detach(pool);
    }

 err:
    if (drbg->seed_pool == NULL)
        VR_rand_pool_free(pool);
    return ret;
}

/*
 * Implements the cleanup_entropy() callback (see VR_RAND_DRBG_set_callbacks())
 *
 */
...
/*
 * Implements the get_nonce() callback (see VR_RAND_DRBG_set_callbacks())
 *
 */
                           ...
                           int entropy, size_t min_len, size_t max_len)
{
    ...
    pool = VR_rand_pool_new(0, min_len, max_len);
    if (pool == NULL)
        return 0;

    if (VR_rand_pool_add_nonce_data(pool) == 0)
        goto err;

    data.instance = drbg;
    VR_CRYPTO_atomic_add(&rand_nonce_count, 1, &data.count, rand_nonce_lock);

    if (VR_rand_pool_add(pool, (unsigned char *)&data, sizeof(data), 0) == 0)
        goto err;
...
/*
 * Implements the cleanup_nonce() callback (see VR_RAND_DRBG_set_callbacks())
 *
 */
...
size_t VR_rand_drbg_get_additional_data(RAND_POOL *pool, unsigned char **pout)
{
    ...
    if (VR_rand_pool_add_additional_data(pool) == 0)
        goto err;
...
{
    VR_rand_pool_reattach(pool, out);
}
...
{
#ifndef OPENSSL_NO_ENGINE
    rand_engine_lock = VR_CRYPTO_THREAD_lock_new();
    if (rand_engine_lock == NULL)
        return 0;
#endif

    rand_meth_lock = VR_CRYPTO_THREAD_lock_new();
    if (rand_meth_lock == NULL)
        goto err1;

    rand_nonce_lock = VR_CRYPTO_THREAD_lock_new();
    if (rand_nonce_lock == NULL)
        goto err2;

    if (!VR_rand_pool_init())
        goto err3;
...
err3:
    VR_CRYPTO_THREAD_lock_free(rand_nonce_lock);
    rand_nonce_lock = NULL;
err2:
    VR_CRYPTO_THREAD_lock_free(rand_meth_lock);
    rand_meth_lock = NULL;
err1:
#ifndef OPENSSL_NO_ENGINE
    VR_CRYPTO_THREAD_lock_free(rand_engine_lock);
...
void VR_RAND_keep_random_devices_open(int keep)
{
    if (RUN_ONCE(&rand_init, do_rand_init))
        VR_rand_pool_keep_random_devices_open(keep);
}

/*
 * VR_RAND_poll() reseeds the default RNG using random input
 *
...
int VR_RAND_poll(void)
{
    ...
    const RAND_METHOD *meth = VR_RAND_get_rand_method();

    if (meth == VR_RAND_OpenSSL()) {
        ...
        ret = VR_rand_drbg_restart(drbg, NULL, 0, 0);
        ...
        pool = VR_rand_pool_new(RAND_DRBG_STRENGTH,
                             RAND_DRBG_STRENGTH / 8,
                             RAND_POOL_MAX_LENGTH);
        if (pool == NULL)
            return 0;

        if (VR_rand_pool_acquire_entropy(pool) == 0)
            goto err;

        if (meth->add == NULL
            || meth->add(VR_rand_pool_buffer(pool),
                         VR_rand_pool_length(pool),
                         (VR_rand_pool_entropy(pool) / 8.0)) == 0)
            goto err;
 ...
 * provided by VR_RAND_add() and VR_RAND_seed() into the <master> DRBG.
 */
...
void VR_rand_pool_free(RAND_POOL *pool)
{
     ...
     * to VR_rand_pool_attach() as `const unsigned char*`.
     * (see corresponding comment in VR_rand_pool_attach()).
     */
    if (!pool->attached)
 ...
 * again to the pool using VR_rand_pool_reattach().
 */
...
{
    pool->buffer = buffer;
    VR_OPENSSL_cleanse(pool->buffer, pool->len);
    pool->len = 0;
}
...
{
    size_t bytes_needed;
    size_t entropy_needed = VR_rand_pool_entropy_needed(pool);
 ...
 *
 * After updating the buffer, VR_rand_pool_add_end() needs to be called
 ...
 * VR_rand_pool_add_begin() (see previous comment).
...
int VR_RAND_set_rand_method(const RAND_METHOD *meth)
{
    ...
    VR_CRYPTO_THREAD_write_lock(rand_meth_lock);
#ifndef OPENSSL_NO_ENGINE
    VR_ENGINE_finish(funct_ref);
    funct_ref = NULL;
#endif
    default_RAND_meth = meth;
    VR_CRYPTO_THREAD_unlock(rand_meth_lock);
    return 1;
}
...
const RAND_METHOD *VR_RAND_get_rand_method(void)
{
    ...
    if (default_RAND_meth == NULL) {
        ...
        if ((e = VR_ENGINE_get_default_RAND()) != NULL
                && (tmp_meth = VR_ENGINE_get_RAND(e)) != NULL) {
            funct_ref = e;
...
int VR_RAND_set_rand_engine(ENGINE *engine)
{
    ...
    if (engine != NULL) {
        if (!VR_ENGINE_init(engine))
            return 0;
        tmp_meth = VR_ENGINE_get_RAND(engine);
        if (tmp_meth == NULL) {
            VR_ENGINE_finish(engine);
            return 0;
        }
    }
    VR_CRYPTO_THREAD_write_lock(rand_engine_lock);
    /* This function releases any prior ENGINE so call it first */
    VR_RAND_set_rand_method(tmp_meth);
    funct_ref = engine;
    VR_CRYPTO_THREAD_unlock(rand_engine_lock);
    return 1;
}
#endif
 ...
 * the default method, then just call VR_RAND_bytes().  Otherwise make
...
int VR_RAND_priv_bytes(unsigned char *buf, int num)
{
    ...
    if (meth != VR_RAND_OpenSSL())
        return VR_RAND_bytes(buf, num);

    drbg = VR_RAND_DRBG_get0_private();
    if (drbg == NULL)
        return 0;

    ret = VR_RAND_DRBG_bytes(drbg, buf, num);
    return ret;
}
...
int VR_RAND_pseudo_bytes(unsigned char *buf, int num)
{
    const RAND_METHOD *meth = VR_RAND_get_rand_method();
