 ...
 * we register it with the "VR_engine_cleanup_int()" stack to be called during
 * cleanup.
 */
...
static int engine_list_add(ENGINE *e)
{
    ...
    if (engine_list_head == NULL) {
         ...
         */
        VR_engine_cleanup_add_last(engine_list_cleanup);
    } else {
...
ENGINE *VR_ENGINE_by_id(const char *id)
{
    ...
    if (strcmp(id, "dynamic")) {
        if ((load_dir = VR_ossl_safe_getenv("OPENSSL_ENGINES")) == NULL)
            load_dir = ENGINESDIR;
        iterator = VR_ENGINE_by_id("dynamic");
        if (!iterator || !VR_ENGINE_ctrl_cmd_string(iterator, "ID", id, 0) ||
            !VR_ENGINE_ctrl_cmd_string(iterator, "DIR_LOAD", "2", 0) ||
            !VR_ENGINE_ctrl_cmd_string(iterator, "DIR_ADD",
                                    load_dir, 0) ||
            !VR_ENGINE_ctrl_cmd_string(iterator, "LIST_ADD", "1", 0) ||
            !VR_ENGINE_ctrl_cmd_string(iterator, "LOAD", NULL, 0))
            goto notfound;
        return iterator;
    }
 notfound:
    VR_ENGINE_free(iterator);
    ENGINEerr(ENGINE_F_ENGINE_BY_ID, ENGINE_R_NO_SUCH_ENGINE);
    VR_ERR_add_error_data(2, "id=", id);
    return NULL;
