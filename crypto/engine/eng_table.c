...
static int int_table_check(ENGINE_TABLE **t, int create)
{
    ...
    if (!create)
        return 0;
    if ((lh = lh_VR_ENGINE_PILE_new(engine_pile_hash, engine_pile_cmp)) == NULL)
        return 0;
    *t = (ENGINE_TABLE *)lh;
    return 1;
}
                          ...
                          int setdefault)
{
    ...
    if (added)
        ...
        VR_engine_cleanup_add_first(cleanup);
    while (num_nids--) {
        tmplate.nid = *nids;
         ...
         */
        if (!sk_VR_ENGINE_push(fnd->sk, e))
            goto end;
        /* "touch" this ENGINE_PILE */
        fnd->uptodate = 0;
        if (setdefault) {
            if (!VR_engine_unlocked_init(e)) {
                ENGINEerr(ENGINE_F_ENGINE_TABLE_REGISTER,
                          ENGINE_R_INIT_FAILED);
                goto end;
            }
            if (fnd->funct)
                VR_engine_unlocked_finish(fnd->funct, 0);
            fnd->funct = e;
            fnd->uptodate = 1;
        }
        nids++;
...
static void int_unregister_cb(ENGINE_PILE *pile, ENGINE *e)
{
    ...
    while ((n = sk_VR_ENGINE_find(pile->sk, e)) >= 0) {
        (void)sk_ENGINE_delete(pile->sk, n);
        pile->uptodate = 0;
    }
    if (pile->funct == e) {
        VR_engine_unlocked_finish(e, 0);
        pile->funct = NULL;
    }
}
...
static void int_cleanup_cb_doall(ENGINE_PILE *p)
{
    if (!p)
        return;
    sk_VR_ENGINE_free(p->sk);
    if (p->funct)
        VR_engine_unlocked_finish(p->funct, 0);
    VR_OPENSSL_free(p);
}
...
void VR_engine_table_cleanup(ENGINE_TABLE **table)
{
    ...
    if (*table) {
        ...
        lh_VR_ENGINE_PILE_free(&(*table)->piles);
        *table = NULL;
    }
    VR_CRYPTO_THREAD_unlock(global_engine_lock);
}
...
#endif
{
    ...
    if (!fnd)
        goto end;
    if (fnd->funct && VR_engine_unlocked_init(fnd->funct)) {
#ifdef ENGINE_TABLE_DEBUG
 ...
 trynext:
    ...
    if ((ret->funct_ref > 0) || !(table_flags & ENGINE_TABLE_FLAG_NOINIT))
        initres = VR_engine_unlocked_init(ret);
    else
        initres = 0;
    if (initres) {
        /* Update 'funct' */
        if ((fnd->funct != ret) && VR_engine_unlocked_init(ret)) {
