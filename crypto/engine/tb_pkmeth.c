...
{
    VR_engine_table_unregister(&pkey_meth_table, e);
}

static void engine_unregister_all_pkey_meths(void)
{
    VR_engine_table_cleanup(&pkey_meth_table);
}
...
int VR_ENGINE_register_pkey_meths(ENGINE *e)
{
    if (e->pkey_meths) {
        ...
        if (num_nids > 0)
            return VR_engine_table_register(&pkey_meth_table,
...
{
    return VR_engine_table_select(&pkey_meth_table, nid);
}
...
{
    EVP_PKEY_METHOD *ret;
    ENGINE_PKEY_METHS_PTR fn = VR_ENGINE_get_pkey_meths(e);
