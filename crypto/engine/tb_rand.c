...
{
    VR_engine_table_unregister(&rand_table, e);
}

static void engine_unregister_all_RAND(void)
{
    VR_engine_table_cleanup(&rand_table);
}

int VR_ENGINE_register_RAND(ENGINE *e)
{
    if (e->rand_meth)
        return VR_engine_table_register(&rand_table,
...
{
    return VR_engine_table_select(&rand_table, dummy_nid);
}
