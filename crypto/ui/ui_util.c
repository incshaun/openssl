                           ...
                           int verify)
{
    ...
    ret =
        VR_UI_UTIL_read_pw(buf, buff, (length > BUFSIZ) ? BUFSIZ : length,
                        prompt, verify);
                    ...
                    int verify)
{
    ...
    ui = VR_UI_new();
    if (ui != NULL) {
        ok = VR_UI_add_input_string(ui, prompt, 0, buf, 0, size - 1);
        if (ok >= 0 && verify)
            ok = VR_UI_add_verify_string(ui, prompt, 0, buff, 0, size - 1, buf);
        if (ok >= 0)
            ok = VR_UI_process(ui);
        VR_UI_free(ui);
    }
...
{
    ui_method_data_index = VR_CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_UI_METHOD,
    ...
    case UIT_PROMPT:
        {
            ...
            const struct pem_password_cb_data *data =
                VR_UI_method_get_ex_data(VR_UI_get_method(ui), ui_method_data_index);
            int maxsize = VR_UI_get_result_maxsize(uis);
            int len = data->cb(result,
                               maxsize > PEM_BUFSIZE ? PEM_BUFSIZE : maxsize,
                               data->rwflag, VR_UI_get0_user_data(ui));

            if (len >= 0)
                result[len] = '\0';
            if (len <= 0)
                return len;
            if (VR_UI_set_result_ex(ui, uis, result, len) >= 0)
                return 1;
            return 0;
        }
    case UIT_VERIFY:
...
UI_METHOD *VR_UI_UTIL_wrap_read_pem_callback(pem_password_cb *cb, int rwflag)
{
    ...
    if ((data = OPENSSL_zalloc(sizeof(*data))) == NULL
        || (ui_method = VR_UI_create_method("PEM password callback wrapper")) == NULL
        || VR_UI_method_set_opener(ui_method, ui_open) < 0
        || VR_UI_method_set_reader(ui_method, ui_read) < 0
        || VR_UI_method_set_writer(ui_method, ui_write) < 0
        || VR_UI_method_set_closer(ui_method, ui_close) < 0
