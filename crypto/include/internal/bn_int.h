...
BIGNUM *VR_bn_expand2(BIGNUM *a, int words);
 ...
 */
/*
 * |num_words| is int because VR_bn_expand2 takes an int. This is an internal
 ...
 * is customarily arranged by VR_bn_correct_top. Output from below functions
 * is not processed with VR_bn_correct_top, and for this reason it may not be
...
int VR_bn_to_mont_fixed_top(BIGNUM *r, const BIGNUM *a, BN_MONT_CTX *mont,
                         BN_CTX *ctx);
int VR_bn_from_mont_fixed_top(BIGNUM *r, const BIGNUM *a, BN_MONT_CTX *mont,
                           BN_CTX *ctx);
...
int VR_bn_div_fixed_top(BIGNUM *dv, BIGNUM *rem, const BIGNUM *m,
