...
static DSO *VR_DSO_new_method(DSO_METHOD *meth)
{
    DSO *ret;
    ...
    if (default_DSO_meth == NULL) {
         ...
         */
        default_DSO_meth = VR_DSO_METHOD_openssl();
    }
    ...
    ret->meth_data = sk_VR_void_new_null();
...
DSO *VR_DSO_load(DSO *dso, const char *filename, DSO_METHOD *meth, int flags)
{
    ...
    if (dso == NULL) {
        ...
        if (VR_DSO_ctrl(ret, DSO_CTRL_SET_FLAGS, flags, NULL) < 0) {
     ...
     */
    if (filename != NULL)
        if (!VR_DSO_set_filename(ret, filename)) {
 ...
 * error because possible VR_DSO_ctrl() commands may return values such as
...
DSO *VR_DSO_dsobyaddr(void *addr, int flags)
{
    DSO *ret = NULL;
    char *filename = NULL;
    int len = VR_DSO_pathbyaddr(addr, NULL, 0);
