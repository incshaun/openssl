...
{
    registry_lock = VR_CRYPTO_THREAD_lock_new();
...
{
    return VR_OPENSSL_LH_strhash(v->scheme);
}
...
int VR_ossl_store_register_loader_int(OSSL_STORE_LOADER *loader)
{
    ...
    if (*scheme != '\0') {
        OSSL_STOREerr(OSSL_STORE_F_OSSL_STORE_REGISTER_LOADER_INT,
                      OSSL_STORE_R_INVALID_SCHEME);
        VR_ERR_add_error_data(2, "scheme=", loader->scheme);
        return 0;
    }
    ...
    VR_CRYPTO_THREAD_write_lock(registry_lock);

    if (loader_register == NULL) {
        loader_register = lh_VR_OSSL_STORE_LOADER_new(store_loader_hash,
                                                   store_loader_cmp);
    }
    ...
    VR_CRYPTO_THREAD_unlock(registry_lock);

    return ok;
}
int VR_OSSL_STORE_register_loader(OSSL_STORE_LOADER *loader)
{
    if (!VR_ossl_store_init_once())
        return 0;
    return VR_ossl_store_register_loader_int(loader);
}
...
const OSSL_STORE_LOADER *VR_ossl_store_get0_loader_int(const char *scheme)
{
    ...
    if (loader == NULL) {
        ...
        VR_ERR_add_error_data(2, "scheme=", scheme);
    }
...
OSSL_STORE_LOADER *VR_OSSL_STORE_unregister_loader(const char *scheme)
{
    if (!VR_ossl_store_init_once())
        return 0;
    return VR_ossl_store_unregister_loader_int(scheme);
}

void VR_ossl_store_destroy_loaders_int(void)
{
    assert(lh_OSSL_STORE_LOADER_num_items(loader_register) == 0);
    lh_VR_OSSL_STORE_LOADER_free(loader_register);
    loader_register = NULL;
    VR_CRYPTO_THREAD_lock_free(registry_lock);
    registry_lock = NULL;
}
