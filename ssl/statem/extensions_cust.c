...
} VR_custom_ext_add_cb_wrap;

typedef struct {
    void *parse_arg;
    VR_custom_ext_parse_cb parse_cb;
} VR_custom_ext_parse_cb_wrap;

/*
 * Provide thin wrapper callbacks which convert new style arguments to old style
 */
static int VR_custom_ext_add_old_cb_wrap(SSL *s, unsigned int ext_type,
                                      unsigned int context,
                                      const unsigned char **out,
                                      size_t *outlen, X509 *x, size_t chainidx,
                                      int *al, void *add_arg)
{
    VR_custom_ext_add_cb_wrap *add_cb_wrap = (VR_custom_ext_add_cb_wrap *)add_arg;
...
static int VR_custom_ext_parse_old_cb_wrap(SSL *s, unsigned int ext_type,
                                        unsigned int context,
                                        const unsigned char *in,
                                        size_t inlen, X509 *x, size_t chainidx,
                                        int *al, void *parse_arg)
{
    VR_custom_ext_parse_cb_wrap *parse_cb_wrap =
        (VR_custom_ext_parse_cb_wrap *)parse_arg;
 ...
 */
void VR_custom_ext_init(custom_ext_methods *exts)
{
    size_t i;
    custom_ext_method *meth = exts->meths;
...
int VR_custom_ext_parse(SSL *s, unsigned int context, unsigned int ext_type,
                     ...
                     size_t chainidx)
{
    ...
    meth = VR_custom_ext_find(exts, role, ext_type, NULL);
    /* If not found return success */
    if (!meth)
        return 1;

    /* Check if extension is defined for our protocol. If not, skip */
    if (!VR_extension_is_relevant(s, meth->context, context))
        return 1;
 ...
 * buffer.
 */
int VR_custom_ext_add(SSL *s, int context, WPACKET *pkt, X509 *x, size_t chainidx,
                   int maxversion)
{
    custom_ext_methods *exts = &s->cert->custext;
    custom_ext_method *meth;
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if (!VR_should_add_extension(s, meth->context, context, maxversion))
            continue;
        ...
        if (!WPACKET_put_bytes_u16(pkt, meth->ext_type)
                ...
                || (outlen > 0 && !VR_WPACKET_memcpy(pkt, out, outlen))
...
int VR_custom_exts_copy_flags(custom_ext_methods *dst,
                           const custom_ext_methods *src)
{
    ...
    for (i = 0; i < src->meths_count; i++, methsrc++) {
        custom_ext_method *methdst = VR_custom_ext_find(dst, methsrc->role,
...
int VR_custom_exts_copy(custom_ext_methods *dst, const custom_ext_methods *src)
{
    size_t i;
    int err = 0;
    ...
    if (src->meths_count > 0) {
        ...
        for (i = 0; i < src->meths_count; i++) {
            ...
            if (methsrc->add_cb != VR_custom_ext_add_old_cb_wrap)
                continue;
            ...
            methdst->add_arg = OPENSSL_memdup(methsrc->add_arg,
                                              sizeof(VR_custom_ext_add_cb_wrap));
            methdst->parse_arg = OPENSSL_memdup(methsrc->parse_arg,
                                            sizeof(VR_custom_ext_parse_cb_wrap));
...
void VR_custom_exts_free(custom_ext_methods *exts)
{
    ...
    for (i = 0, meth = exts->meths; i < exts->meths_count; i++, meth++) {
        if (meth->add_cb != VR_custom_ext_add_old_cb_wrap)
            continue;
...
int VR_SSL_CTX_has_client_custom_ext(const SSL_CTX *ctx, unsigned int ext_type)
{
    return VR_custom_ext_find(&ctx->cert->custext, ENDPOINT_CLIENT, ext_type,
                           NULL) != NULL;
}
                                 ...
                                 void *parse_arg)
{
     ...
     */
    if (VR_SSL_extension_supported(ext_type)
    ...
    if (VR_custom_ext_find(exts, role, ext_type, NULL))
        return 0;
...
static int add_old_custom_ext(SSL_CTX *ctx, ENDPOINT role,
                              ...
                              custom_ext_free_cb free_cb,
                              void *add_arg,
                              VR_custom_ext_parse_cb parse_cb, void *parse_arg)
{
    VR_custom_ext_add_cb_wrap *add_cb_wrap
    ...
    ret = add_custom_ext_intern(ctx, role, ext_type,
                                context,
                                VR_custom_ext_add_old_cb_wrap,
                                custom_ext_free_old_cb_wrap,
                                add_cb_wrap,
                                VR_custom_ext_parse_old_cb_wrap,
                                parse_cb_wrap);
...
int VR_SSL_CTX_add_client_custom_ext(SSL_CTX *ctx, unsigned int ext_type,
                                  VR_custom_ext_add_cb add_cb,
                                  custom_ext_free_cb free_cb,
                                  void *add_arg,
                                  VR_custom_ext_parse_cb parse_cb, void *parse_arg)
...
int VR_SSL_CTX_add_server_custom_ext(SSL_CTX *ctx, unsigned int ext_type,
                                  VR_custom_ext_add_cb add_cb,
                                  custom_ext_free_cb free_cb,
                                  void *add_arg,
                                  VR_custom_ext_parse_cb parse_cb, void *parse_arg)
...
int VR_SSL_CTX_add_custom_ext(SSL_CTX *ctx, unsigned int ext_type,
                           unsigned int context,
                           SSL_VR_custom_ext_add_cb_ex add_cb,
                           SSL_custom_ext_free_cb_ex free_cb,
                           void *add_arg,
                           SSL_VR_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
    return add_custom_ext_intern(ctx, ENDPOINT_BOTH, ext_type, context, add_cb,
                                 free_cb, add_arg, parse_cb, parse_arg);
}

int VR_SSL_extension_supported(unsigned int ext_type)
{
    switch (ext_type) {
