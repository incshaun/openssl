...
# define EVP_PKEY_CTX_set_rsa_padding(ctx, pad) \
        VR_RSA_pkey_ctx_ctrl(ctx, -1, EVP_PKEY_CTRL_RSA_PADDING, pad, NULL)

# define EVP_PKEY_CTX_get_rsa_padding(ctx, ppad) \
        VR_RSA_pkey_ctx_ctrl(ctx, -1, EVP_PKEY_CTRL_GET_RSA_PADDING, 0, ppad)

# define EVP_PKEY_CTX_set_rsa_pss_saltlen(ctx, len) \
        VR_RSA_pkey_ctx_ctrl(ctx, (EVP_PKEY_OP_SIGN|EVP_PKEY_OP_VERIFY), \
...
# define EVP_PKEY_CTX_set_rsa_pss_keygen_saltlen(ctx, len) \
        VR_EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_RSA_PSS, EVP_PKEY_OP_KEYGEN, \
                          EVP_PKEY_CTRL_RSA_PSS_SALTLEN, len, NULL)
...
# define EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, bits) \
        VR_RSA_pkey_ctx_ctrl(ctx, EVP_PKEY_OP_KEYGEN, \
...
# define  EVP_PKEY_CTX_set_rsa_mgf1_md(ctx, md) \
        VR_RSA_pkey_ctx_ctrl(ctx, EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT, \
...
# define  EVP_PKEY_CTX_set_rsa_oaep_md(ctx, md) \
        VR_EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_RSA, EVP_PKEY_OP_TYPE_CRYPT,  \
...
# define  EVP_PKEY_CTX_set_rsa_pss_keygen_md(ctx, md) \
        VR_EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_RSA_PSS,  \
...
# define RSA_set_app_data(s,arg)         VR_RSA_set_ex_data(s,0,arg)
# define RSA_get_app_data(s)             VR_RSA_get_ex_data(s,0)

RSA *VR_RSA_new(void);
RSA *VR_RSA_new_method(ENGINE *engine);
int VR_RSA_bits(const RSA *rsa);
int VR_RSA_size(const RSA *rsa);
...
int VR_RSA_get0_multi_prime_crt_params(const RSA *r, const BIGNUM *exps[],
                                    const BIGNUM *coeffs[]);
const BIGNUM *VR_RSA_get0_n(const RSA *d);
const BIGNUM *VR_RSA_get0_e(const RSA *d);
const BIGNUM *VR_RSA_get0_d(const RSA *d);
const BIGNUM *VR_RSA_get0_p(const RSA *d);
const BIGNUM *VR_RSA_get0_q(const RSA *d);
...
/* Deprecated version */
DEPRECATEDIN_0_9_8(RSA *VR_RSA_generate_key(int bits, unsigned long e, void
                                         (*callback) (int, int, void *),
                                         void *cb_arg))

/* New version */
int VR_RSA_generate_key_ex(RSA *rsa, int bits, BIGNUM *e, BN_GENCB *cb);
/* Multi-prime version */
...
int VR_RSA_check_key(const RSA *);
int VR_RSA_check_key_ex(const RSA *, BN_GENCB *cb);
        /* next 4 return -1 on error */
int VR_RSA_public_encrypt(int flen, const unsigned char *from,
                       unsigned char *to, RSA *rsa, int padding);
int VR_RSA_private_encrypt(int flen, const unsigned char *from,
                        unsigned char *to, RSA *rsa, int padding);
int VR_RSA_public_decrypt(int flen, const unsigned char *from,
                       unsigned char *to, RSA *rsa, int padding);
int VR_RSA_private_decrypt(int flen, const unsigned char *from,
                        unsigned char *to, RSA *rsa, int padding);
void VR_RSA_free(RSA *r);
/* "up" the RSA object's reference count */
int VR_RSA_up_ref(RSA *r);

int VR_RSA_flags(const RSA *r);
...
int VR_RSA_print_fp(FILE *fp, const RSA *r, int offset);
# endif

int VR_RSA_print(BIO *bp, const RSA *r, int offset);

/*
 * The following 2 functions sign and verify a X509_SIG ASN1 object inside
 * PKCS#1 padded RSA encryption
 */
int VR_RSA_sign(int type, const unsigned char *m, unsigned int m_length,
             unsigned char *sigret, unsigned int *siglen, RSA *rsa);
int VR_RSA_verify(int type, const unsigned char *m, unsigned int m_length,
...
int VR_RSA_sign_ASN1_OCTET_STRING(int type,
                               ...
                               RSA *rsa);
int VR_RSA_verify_ASN1_OCTET_STRING(int type, const unsigned char *m,
                                 unsigned int m_length, unsigned char *sigbuf,
                                 unsigned int siglen, RSA *rsa);

int VR_RSA_blinding_on(RSA *rsa, BN_CTX *ctx);
void VR_RSA_blinding_off(RSA *rsa);
BN_BLINDING *VR_RSA_setup_blinding(RSA *rsa, BN_CTX *ctx);
...
int VR_RSA_padding_check_PKCS1_type_2(unsigned char *to, int tlen,
                                   const unsigned char *f, int fl,
                                   int rsa_len);
int VR_PKCS1_MGF1(unsigned char *mask, long len, const unsigned char *seed,
...
int VR_RSA_padding_check_PKCS1_OAEP_mgf1(unsigned char *to, int tlen,
                                      ...
                                      const EVP_MD *mgf1md);
int VR_RSA_padding_add_SSLv23(unsigned char *to, int tlen,
                           const unsigned char *f, int fl);
int VR_RSA_padding_check_SSLv23(unsigned char *to, int tlen,
                             const unsigned char *f, int fl, int rsa_len);
int VR_RSA_padding_add_none(unsigned char *to, int tlen, const unsigned char *f,
                         int fl);
int VR_RSA_padding_check_none(unsigned char *to, int tlen,
                           const unsigned char *f, int fl, int rsa_len);
int VR_RSA_padding_add_X931(unsigned char *to, int tlen, const unsigned char *f,
                         int fl);
int VR_RSA_padding_check_X931(unsigned char *to, int tlen,
                           const unsigned char *f, int fl, int rsa_len);
int VR_RSA_X931_hash_id(int nid);
...
#define RSA_get_ex_new_index(l, p, newf, dupf, freef) \
    VR_CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_RSA, l, p, newf, dupf, freef)
...
int (*VR_RSA_meth_get_pub_enc(const RSA_METHOD *meth))
    (int flen, const unsigned char *from,
     unsigned char *to, RSA *rsa, int padding);
int VR_RSA_meth_set_pub_enc(RSA_METHOD *rsa,
...
int (*VR_RSA_meth_get_pub_dec(const RSA_METHOD *meth))
    (int flen, const unsigned char *from,
     unsigned char *to, RSA *rsa, int padding);
int VR_RSA_meth_set_pub_dec(RSA_METHOD *rsa,
...
int (*VR_RSA_meth_get_priv_enc(const RSA_METHOD *meth))
    (int flen, const unsigned char *from,
     unsigned char *to, RSA *rsa, int padding);
int VR_RSA_meth_set_priv_enc(RSA_METHOD *rsa,
...
int VR_RSA_meth_set_init(RSA_METHOD *rsa, int (*init) (RSA *rsa));
int (*VR_RSA_meth_get_finish(const RSA_METHOD *meth)) (RSA *rsa);
int VR_RSA_meth_set_finish(RSA_METHOD *rsa, int (*finish) (RSA *rsa));
int (*VR_RSA_meth_get_sign(const RSA_METHOD *meth))
    (int type,
     const unsigned char *m, unsigned int m_length,
     unsigned char *sigret, unsigned int *siglen,
     const RSA *rsa);
int VR_RSA_meth_set_sign(RSA_METHOD *rsa,
...
int VR_RSA_meth_set_verify(RSA_METHOD *rsa,
...
int VR_RSA_meth_set_keygen(RSA_METHOD *rsa,
