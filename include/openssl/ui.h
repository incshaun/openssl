 ...
 * (VR_UI_process()) -2 if interrupted or in some other way cancelled. When
...
/*-
   ...
   is useful when retrieving results with VR_UI_get0_result(). */
 ...
 * VR_UI_add_user_data().  It is not recommended to have more than one input in
...
*/
char *VR_UI_construct_prompt(UI *ui_method,
 ...
 *
 * Note that the VR_UI_OpenSSL() method completely ignores the user data. Other
...
/* The commands */
/*
 * Use UI_CONTROL_PRINT_ERRORS with the value 1 to have VR_UI_process print the
 * OpenSSL error stack before printing any info or added error messages and
 * before any prompting.
 */
# define UI_CTRL_PRINT_ERRORS            1
/*
 * Check if a VR_UI_process() is possible to do again with the same instance of
 * a user interface.  This makes VR_UI_ctrl() return 1 if it is redoable, and 0
 * if not.
 */
# define UI_CTRL_IS_REDOABLE             2

/* Some methods may use extra data */
# define UI_set_app_data(s,arg)         VR_UI_set_ex_data(s,0,arg)
# define UI_get_app_data(s)             VR_UI_get_ex_data(s,0)

# define UI_get_ex_new_index(l, p, newf, dupf, freef) \
    VR_CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_UI, l, p, newf, dupf, freef)
 ...
 */
const UI_METHOD *VR_UI_null(void);
...
int VR_UI_UTIL_read_pw_string(char *buf, int length, const char *prompt,
                           int verify);
int VR_UI_UTIL_read_pw(char *buf, char *buff, int size, const char *prompt,
                    int verify);
UI_METHOD *VR_UI_UTIL_wrap_read_pem_callback(pem_password_cb *cb, int rwflag);
