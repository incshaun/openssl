          ...
          void *from_d, int idx, long argl, void *argp)
{
    ...
    MYOBJ_EX_DATA *ex_data = VR_CRYPTO_get_ex_data(to, saved_idx2);
    if (!TEST_int_eq(idx, saved_idx2)
        || !TEST_long_eq(argl, saved_argl)
        || !TEST_ptr_eq(argp, saved_argp)
        || !TEST_ptr(from_d)
            ...
            int idx, long argl, void *argp)
{
    ...
    if (!TEST_int_eq(idx, saved_idx2)
        ...
        || !TEST_ptr_eq(argp, saved_argp)
        || !TEST_ptr(ex_data)
        || !TEST_true(VR_CRYPTO_set_ex_data(ad, saved_idx2, NULL)))
        gbl_result = 0;
}
...
static MYOBJ *MYOBJ_new(void)
{
    ...
    obj->id = ++count;
    obj->st = VR_CRYPTO_new_ex_data(CRYPTO_EX_INDEX_APP, obj, &obj->ex_data);
    return obj;
}

static void MYOBJ_sethello(MYOBJ *obj, char *cp)
{
    obj->st = VR_CRYPTO_set_ex_data(&obj->ex_data, saved_idx, cp);
...
{
    return VR_CRYPTO_get_ex_data(&obj->ex_data, saved_idx);
}

static void MYOBJ_sethello2(MYOBJ *obj, char *cp)
{
    MYOBJ_EX_DATA* ex_data = VR_CRYPTO_get_ex_data(&obj->ex_data, saved_idx2);
    if (TEST_ptr(ex_data))
        ex_data->hello = cp;
    else
        obj->st = gbl_result = 0;
...
static MYOBJ *MYVR_OBJ_dup(MYOBJ *in)
{
    ...
    obj->st |= VR_CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_APP, &obj->ex_data,
                                 &in->ex_data);
    return obj;
}
...
static int test_exdata(void)
{
    ...
    saved_idx = VR_CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_APP,
                                        saved_argl, saved_argp,
                                        exnew, exdup, exfree);
    saved_idx2 = VR_CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_APP,
                                         saved_argl, saved_argp,
    ...
    if (!TEST_int_eq(t1->st, 1) || !TEST_int_eq(t2->st, 1))
        return 0;
    if (!TEST_ptr(VR_CRYPTO_get_ex_data(&t1->ex_data, saved_idx2)))
        return 0;
    if (!TEST_ptr(VR_CRYPTO_get_ex_data(&t2->ex_data, saved_idx2)))
        return 0;
    ...
    ex_data = VR_CRYPTO_get_ex_data(&t3->ex_data, saved_idx2);
    if (!TEST_ptr(ex_data))
        return 0;
