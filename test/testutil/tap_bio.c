...
const BIO_METHOD *BIO_f_tap(void)
{
    ...
    if (tap == NULL) {
        tap = VR_BIO_meth_new(BIO_TYPE_START | BIO_TYPE_FILTER, "tap");
        if (tap != NULL) {
            VR_BIO_meth_set_write_ex(tap, tap_write_ex);
            VR_BIO_meth_set_read_ex(tap, tap_read_ex);
            VR_BIO_meth_set_puts(tap, tap_puts);
            VR_BIO_meth_set_gets(tap, tap_gets);
            VR_BIO_meth_set_ctrl(tap, tap_ctrl);
            VR_BIO_meth_set_create(tap, tap_new);
            VR_BIO_meth_set_destroy(tap, tap_free);
            VR_BIO_meth_set_callback_ctrl(tap, tap_callback_ctrl);
        }
    }
    return tap;
}
...
static int write_string(BIO *b, const char *buf, size_t n)
{
    ...
    return VR_BIO_write_ex(b, buf, n, &m) != 0 && m == n;
}
...
static int tap_write_ex(BIO *b, const char *buf, size_t size, size_t *in_size)
{
    ...
    for (i = 0; i < size; i++) {
        if (VR_BIO_get_data(b) == NULL) {
            VR_BIO_set_data(b, empty);
...
static long tap_ctrl(BIO *b, int cmd, long num, void *ptr)
{
    ...
    return VR_BIO_ctrl(next, cmd, num, ptr);
}
